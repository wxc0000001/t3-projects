...
<head><title>行家信息管理</title>
<script>  
  var arr ;
  var arr_time;
  var record_id;
  function locking(button_type){//添加和修改界面浮层锁定
     ...
     if(button_type!="add"){
      ...
      $.ajax({
                ...
                type: "post",
                data: {"tr_id":tr_id,"table_name":"expert_info"},
                dataType: "text",
                //jsonp: "callbackJsonp", // 传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名
                //timeout:10000,
                success: function(data) {
                  if(data!="读取信息失败!"){//加载修改信息
                      arr=data.split("##");                      
                      document.getElementById("name").value=arr[1];  
                      document.getElementById("profile").value=arr[2];
                      document.getElementById("img").value=arr[4].substring(arr[4].lastIndexOf("/")+1,arr[4].length);
                      document.getElementById("article_url").value=arr[5];
                      document.getElementById("user_open_id").value=arr[6];
...
$mysql_info = json_decode(file_get_contents("json/mysql_info.json"));
  ...
  $sql="select * from expert_info order by id desc";
  $result = mysql_query($sql,$con);
  //$row=mysql_fetch_array($result);
  //$num_rows=mysql_num_rows($result);
  $sql_="select id,label_name from activity_label";//取标签id和名称
                    ...
                    <table class="table no-margin" style="table-layout:fixed;word-wrap:break-word;">
                      <thead>
                        <tr>
                          ...
                          <th style="width:10%;text-align:center;">简介</th>
                          <th style="width:10%;text-align:center;">标签</th>
                          <th style="width:20%;text-align:center;">文章链接</th>
            						  <th style="width:20%;text-align:center;">用户id</th>
            						  <th style="width:8.5%;text-align:center;">头像</th>
                      ...
                      <tbody id="table2">
                        ...
                        <td style="vertical-align:middle;"><?php echo $row["profile"];?></td>
                        <!-- <td><?php echo $row["label"];?></td> -->
                        <td style="vertical-align:middle;"><?php 
                          $sql_label="select label_name from activity_label where id in (".$row["label"].")";//根据记录中标签的id取名称
                          ...
                          echo implode(" | ",$temp);
                        ?></td>
                        <td style="vertical-align:middle;"><?php echo $row["article_url"];?></td>
               ...
               <a href=JavaScript:; class="STYLE1" onclick="Lock_CheckForm(this);">[关闭]</a>     
                  ...
                  <div class="widget-body">
                    <form class="form-horizontal no-margin" >
                      ...
                      <div class="form-group">
                        <label for="emailId" class="col-sm-2 control-label">行家简介</label>
                        <div class="col-sm-10">
                          <input type="text" class="form-control" id="profile" placeholder="简介">
                        </div>
                      </div>
                      <div class="form-group">
                        <label for="pwd" class="col-sm-2 control-label">行家标签</label>
                        <div class="col-sm-10">
                          <?php for($i=0;$i<$num_rows;$i++) {?>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-10">
                          <input type="text" class="form-control" id="article_url"  placeholder="链接">
					             ...
					             <div class="form-group">
                        <label for="pwd" class="col-sm-2 control-label">行家类别</label>
                      ...
                      <div class="form-group">
                        <label for="rptPwd" class="col-sm-2 control-label">行家头像</label>
                        <div class="col-sm-10" >    
...
$(function(){ 
  $('#uploadify').uploadify({
    ...
    'formData':{'targetFolder':'fivekids/img_upload/expert'},
    ...
    onUploadSuccess:function(file,data,response){//上传成功，加载预览
      //alert(data);
      document.getElementById('img').value=file.name; 
      imgObj="<p><img width='100px' height='100px' src='http://t3china.t3group.cn/fivekids/img_upload/expert/"+file.name+"'></p>"
      $("#show").empty();
      $("#show").append(imgObj);
    }
  });
...
$("#add").click(function(){ 
	...
	var img_url="http://t3china.t3group.cn/fivekids/img_upload/expert/"+document.getElementById("img").value;
  ...
  var label=result.join(",");
    if(name!=""&&user_open_id!=""){
        $.ajax({
                url: "php/insert_edit_expert.php",
                type: "post",
                data: {"change_type":"add","name":name,"profile":profile,"img_url":img_url,"article_url":article_url,"user_open_id":user_open_id,"label":label,"class":radio_checked},
                dataType: "text",
                //jsonp: "callbackJsonp", // 传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名
                //timeout:10000,
                success: function(data) {
  ...
  var label=result.join(",");
    if(name!=""&&user_open_id!=""){
        $.ajax({
                url: "php/insert_edit_expert.php",
                type: "post",
                data: {"record_id":record_id,"change_type":"edit","name":name,"profile":profile,"img_url":img_url,"article_url":article_url,"user_open_id":user_open_id,"label":label,"class":radio_checked},
                dataType: "text",
                //jsonp: "callbackJsonp", // 传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名
                //timeout:10000,
                success: function(data) {
