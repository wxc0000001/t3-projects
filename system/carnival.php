  ...
  function locking(button_type){  //锁定浮层
     ...
     if(button_type!="add"){
      ...
      $.ajax({
                ...
                type: "post",
                data: {"tr_id":tr_id,"table_name":"carnival"},
                dataType: "text",
...
$mysql_info = json_decode(file_get_contents("json/mysql_info.json"));
  ...
  $sql="select * from carnival order by id desc";
  $result = mysql_query($sql,$con);

  $sql_="select id,label_name from activity_label";//取所有标签id和名称
                  ...
                  <div class="widget-body">
                    ...
                    <table class="table no-margin" style="table-layout:fixed;word-wrap:break-word;">
                      ...
                      <tbody id="table2">
                        ...
                        <td style="vertical-align:middle;"><?php //根据标签id查询标签名称
...
$(function(){
  $('#uploadify').uploadify({
    ...
    'formData':{'targetFolder':'fivekids/img_upload/carnival/'},
    ...
    onUploadSuccess:function(file,data,response){//上传成功，加载图片预览
      ...
      imgObj="<p><img width='100px' height='100px' src='http://t3china.t3group.cn/fivekids/img_upload/carnival/"+file.name+"'></p>"
      $("#show").empty();
      $("#show").append(imgObj);
    }
  });
...
$("#add").click(function(){
  ...
  var img_url="http://t3china.t3group.cn/fivekids/img_upload/carnival/"+document.getElementById("img").value;
  //取选中标签
  var j=0;
   ...
   if(title!=""&&location!=""&&begin_time!=""&&end_time!=""&&document.getElementById("img").value!=""){
      if(begin_time<=end_time){
        $.ajax({
                ...
                type: "post",
                data: {"table_name":"carnival","change_type":"add","title":title,"location":location,"begin_time":begin_time,"end_time":end_time,
                      "img_url":img_url,"content":content,
                      "label":label,"introduction":introduction},
                dataType: "text",
                //jsonp: "callbackJsonp", // 传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名
                //timeout:10000,
                success: function(data) {
   ...
   if(title!=""&&location!=""&&begin_time!=""&&end_time!=""&&document.getElementById("img").value!=""){
      if(begin_time<=end_time){
        $.ajax({
                url: "php/insert_edit_carnival.php",
                type: "post",
                data: {"table_name":"carnival","record_id":record_id,"change_type":"edit","title":title,"location":location,"begin_time":begin_time,"end_time":end_time,
                      "img_url":img_url,"content":content,
                      "label":label,"introduction":introduction},
                dataType: "text",
                //jsonp: "callbackJsonp", // 传递给请求处理程序或页面的，用以获得jsonp回调函数名的参数名
